...
* WAF Bypass: **SQLite**
    - 문자열 검사 우회
        + ```CHAR``` 함수와 ```||```를 이용해 문자열 검사를 우회할 수 있음
            | | 설명 |
            |:---:|------|
            | ```CHAR``` 함수 | ```CHAR(X1, X2, ..., Xn)``` 형식 <br/> &nbsp;&nbsp; → 정수 ```X1```에서 ```Xn```까지의 유니코드 코드포인트 값을 갖는 문자열로 구성된 문자열을 반환함 |
            | ```||``` | SQLite에서 문자열을 합치기 위해 사용함 |
        + 문자열 검사 우회 예시
            ```sql
            SELECT CHAR(0x61); -- 0x61을 유니코드로 갖는 문자열 'a'가 반환됨
            SELECT CHAR(0x61)||CHAR(0x62); -- 0x61을 유니코드로 갖는 문자 'a'와 0x62를 유니코드로 갖는 문자 'b'를 합친 문자열 'ab'가 반환됨
            ```
    - 공백 검사 우회
        ...
        + 공백 검사 우회 예시
            ```sql
            SELECT
            1; -- 결과: 1 (개행으로 공백 검사를 우회함)

            SELECT/**/1; -- 결과: 1 (주석 /**/를 이용해 공백 검사를 우회함)
            ```
    - 구문 검사 우회: *select filter bypass**
        + ```SELECT```를 필터링하고 있어 원하는 값을 반환할 수 없는 경우 SQLite에서는 ```UNION VALUES(num)```을 이용하여 원하는 값을 반환시킬 수 있음
        + 구문 검사 우회 예시
            ```sql
...
### Out of DBMS
* DBMS에서 제공하는 특별한 함수 또는 기능 등을 이용해 파일 시스템, 네트워크, OS 명령어 등에도 접근할 수 있음 <br/> &nbsp;&nbsp; → 이를 통해 SQL Injection으로 단순히 데이터베이스의 정보만 획득하는 것이 아니라 파일 시스템, 네트워크, 시스템 장악까지도 가능할 수도 있음
    - DBMS의 버전이 올라감에 따라 기본적으로 위험한 함수나 기능 등을 제거하거나, 기본 설정/권한으로 접근하지 못하게 하는 등 다양한 방법으로 해당 공격 방식에 대해 패치를 진행하고 있음 ***⇒ DBMS의 버전과 설정에 따라 정상적으로 동작하지 않을 수도 있음***
...
* DBMS 권한 문제 주의사항
    - DBMS 어플리케이션 작동 권한
        + 리눅스 서버는 이용자 별로 권한을 나누어 관리함 *→ 서버에서 DBMS 작동 시에는 **DBMS 전용 계정**(nologin)을 만들어 사용해야 함*
            + root 계정이나 www-data(webservice)와 같이 다른 어플리케이션 권한을 사용하게 될 경우 문제가 발생할 수 있음
            + ⚠️ **계정을 분리하지 않을 경우 DBMS에서 파일 시스템, 네트워크, OS 명령어 등에 접근 가능할 수 있음** <br/> &nbsp;&nbsp; → 파일 시스템 및 네트워크에 접근하거나 OS 명령어를 실행하는 등 추가적인 Side effect가 발생할 수 있음
    - DBMS 계정 권한
        - DBMS 내부에 계정과 권한이 존재함 *→ root 계정의 사용을 지양하고, **서비스/기능 별로 권한이 분리된 계정**을 생성해 사용해야 함*
            + 웹 어플리케이션에서 DBMS의 root 계정을 사용하고, 여러 서비스에서 같은 DBMS를 사용할 때 접속 계정을 분리하지 않는 경우 문제가 발생할 수 있음
            + ⚠️ **서비스 별 계정이 분리되지 않은 경우 SQL Injection이 발생하면 해당 계정에서 접근할 수 있는 데이터베이스가 많아 더 큰 피해가 발생하게 됨**
