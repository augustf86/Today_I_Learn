...
### 시스템 콜 명령어
커널에게 필요한 동작을 요청하는 시스템 콜을 사용하는 명령어
* 시스템 콜(syscall)의 배경
    - 윈도우, 리눅스, 맥 등의 현대 OS는 내부적으로 매우 복잡한 동작을 함
        | | 설명 |
        |:---:|------|
        | 이유 | - 컴퓨터 자원의 효율적인 사용을 위함 <br/> - 사용자에게 편리한 경험을 제공하기 위함 |
        | 특징 | OS는 연결된 모든 하드웨어 및 소프트웨어 접근할 수 있으며, 이들을 제어할 수 있음 <br/> &nbsp;&nbsp; → 📌 **해킹으로부터 이러한 권한을 보호하기 위해 커널 모드와 유저 모드로 권한을 나눔** |
    - 커널 모드와 유저 모드
        | 모드 | 설명 |
        |:---:|------|
        | 커널 모드 | 운영체제가 전체 시스템을 제어하기 위해 시스템 소프트웨어에 부여하는 권한 <br/> &nbsp;&nbsp; - 파일시스템, 입력/출력, 네트워크 통신, 메모리 관리 등 **모든 저수준의 작업**은 사용자 모르게 커널 모드에서 진행됨 <br/> &nbsp;&nbsp; - ⚠️ 시스템의 모든 부분을 제어할 수 있음 → 해커가 커널 모드까지 진입하게 되면 시스템은 거의 무방비 상태가 됨 |
        | 유저 모드 | 운영체제가 사용자에게 부여하는 권한 <br/> &nbsp;&nbsp; - 동영상 시청, 웹, 게임, 프로그래밍 등은 모두 유저 모드에서 이루어짐 <br/> &nbsp;&nbsp; - 리눅스에서 루트 권한으로 사용자를 추가하고 패키지를 내려 받는 행위 등도 모두 유저 모드에서 이뤄짐 <br/> &nbsp;&nbsp; - 📌 해킹이 발생해도 유저 모드의 권한까지 밖에 획득하지 못함 → **해커로부터 커널의 막강한 권한을 보호할 수 있음** |
        - 💡 **시스템 콜**(system call, syscall)
            + 유저 모드에서 커널 모드의 시스템 소프트웨어에게 어떤 동작을 요청하기 위해 사용됨
            + 대다수의 소프트웨어는 커널의 도움을 필요로 하는데, 유저 모드에서는 직접 할 수 없는 부분에 대해서는 커널이 도움을 줘야 함
                - 파일 시스템을 읽거나 상호 네트워크 연결을 맺는 등의 작업 수행 시 **커널의 도움이 필수적임**
            + 📌 시스템 콜의 수행 과정 <br/> &nbsp;&nbsp; : 유저 모드의 소프트웨어가 필요한 도움을 요청함 → 커널이 요청한 동작을 수행하여 유저에게 결과를 반환함
                - 도움이 필요하다는 요청을 "시스템 콜"이라고 부름
* 시스템 콜 명령어의 종류
    | 명령어 형식 | 설명 |
    |---|------|
    | ```syscall``` | 시스켐 콜은 함수로, 필요한 기능과 인자에 대한 정보를 레지스터로 전달하면 커널이 이를 읽어서 요청을 처리함 <br/> - [리눅스, x64 아키텍처] ```rax```로 무슨 요청인지 나타내고, ```rdi → rsi → rdx → rcx → r8 → r9 → stack``` 순서로 <br/> &nbsp;&nbsp; 필요한 인자를 전달함 |
* x64 syscall 테이블 (300여 개 중 일부)
