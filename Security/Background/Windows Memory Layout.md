...
| | 설명 |
|:---:|------|
| 정의 및 설명 | 일시적으로 저장하고 사용하는 임시 영역 <br/> &nbsp;&nbsp; - 보통 지역 변수나 함수의 리턴 주소가 저장됨 <br/> &nbsp;&nbsp; - 윈도우 프로세스의 각 스레드는 자신만의 스택 공간을 가지고 있음|
| 일반적인 권한 | **읽기/쓰기 권한** → 스택 영역은 자유롭게 읽고 쓸 수 있어야 하기 때문 |
| 사용 예시 | <img width="650" alt="스택" src="https://github.com/augustf86/Today_I_Learn/assets/122844932/4e85ef73-589e-4b5e-b8c0-89dc04feadc1"> |

* ❓ 스택에 대해서 '**아래로 자란다**'라는 표현을 사용하는 이유
    - 스택이 확장될 때 기존 주소보다 '낮은 주소'로 확장되기 때문
...
| | 설명 |
|:---:|------|
| 정의 및 설명 | 프로그램이 여러 용도로 사용하기 위해 할당받는 공간 <br/>*→ 모든 종류의 데이터가 저장될 수 있음 = 자유롭게 사용 가능한 공간* |
| 일반적인 권한 | **읽기/쓰기 권한** → 보통은 데이터를 읽고 쓰기만 하기 때문 <br/> &nbsp;&nbsp; - 상황에 따라 실행 권한을 가지는 경우도 존재함 |
| 사용 예시 | <img width="850" alt="힙" src="https://github.com/augustf86/Today_I_Learn/assets/122844932/ca16c108-bd9f-45a5-ac1c-4a1cbc5ef113"> |

* 스택과 비교했을 때 힙이 가지는 특징
    - 비교적 스택보다 큰 데이터도 저장할 수 있음
